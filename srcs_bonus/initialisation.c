...
void	create_environment(t_uni *uni)
{
	...
	if (uni->envp)
	{
		...
		uni->envp = change_in_env("PWD=", getcwd(NULL, 0), uni->envp, 0);
	}
}
...
void	get_environment(t_uni *uni, char **envp)
{
	...
	if (envp[0])
	{
		...
		if (!shlvl_str)
			uni->envp = change_in_env("SHLVL=", ft_itoa('1'), uni->envp, 0);
		else
		{
			shlvl = ft_atoi(shlvl_str) + 1;
			free(shlvl_str);
			uni->envp = change_in_env("SHLVL=", ft_itoa(shlvl), uni->envp, 0);
		}
	}
	else
